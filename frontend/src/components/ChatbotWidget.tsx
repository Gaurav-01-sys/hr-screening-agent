...
export function ChatbotWidget({ candidate, job, response }: ChatbotWidgetProps) {
  ...
  if (!isOpen) {
    return (
      ...
      >
        ...
        <span className="flex size-5 items-center justify-center rounded-full bg-primary-foreground text-[10px] font-bold text-primary" aria-label="New chat available">1</span>
      </Button>
    )
  }
  ...
  return (
    ...
    >
      <header className="flex items-center justify-between border-b border-border bg-muted/25 px-4 py-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h2 className="truncate text-sm font-semibold text-foreground">Recruiter copilot</h2>
            <p className="truncate text-xs text-muted-foreground">Grounded in {candidate.full_name || "this screening"}</p>
          </div>
        </div>
        ...
        {conversation.length === 0 ? (
          <div className="flex h-full flex-col justify-center">
            <div className="mb-5 rounded-lg border border-primary/20 bg-primary/5 p-4">
              ...
              <p className="mt-1 text-xs leading-5 text-muted-foreground">I can explain recorded evidence, requirements, rule outcomes, and interview strategy. I will call out anything missing from the screening memory.</p>
            </div>
          ...
          <div className="space-y-4">
            {conversation.map((item, index) => (
              <div key={`${item.message.timestamp}-${index}`} className={`flex gap-2.5 ${item.message.role === "user" ? "justify-end" : "justify-start"}`}>
                {item.message.role !== "user" && <Bot className="mt-1 size-4 shrink-0 text-primary" aria-hidden="true" />}
                <div className={`max-w-[88%] rounded-lg px-3 py-2.5 text-sm leading-6 ${item.message.role === "user" ? "bg-primary text-primary-foreground" : "border border-border bg-muted/35 text-foreground"}`}>
                  ...
                  {item.sources && item.sources.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-1.5 border-t border-border/70 pt-2">
                      {item.sources.map((source) => <Badge key={source} variant="outline" className="max-w-full font-mono text-[10px]">{source}</Badge>)}
                    </div>
                  )}
                </div>
                {item.message.role === "user" && <UserRound className="mt-1 size-4 shrink-0 text-muted-foreground" aria-hidden="true" />}
              </div>
            ))}
            {isLoading && (
      ...
      {error && <p className="border-t border-destructive/20 bg-destructive/5 px-4 py-2 text-xs text-destructive" role="alert">{error}</p>}
